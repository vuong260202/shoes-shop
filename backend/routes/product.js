...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    ...
    cb(null, path.join(__dirname, '..', '..', 'Frontend','shoe-shop','public','img'));
  },
...
router.post("/update-product", async (req, res) => {
  ...
  if (req.body.size) {
    change.size = req.body.size;
  }

  if (req.body.total) {
    change.total = req.body.total;
  }

  if (req.body.price) {
    change.price = req.body.price;
  }

  console.log(change);

  try {
    let product = await global.sequelizeModels.Product.update(change, {
      where: {
        id: req.body.id,
      },
    });
